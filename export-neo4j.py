...
with pyodbc.connect('DRIVER='+driver+';SERVER=tcp:'+server+';PORT=1433;DATABASE='+database+';UID='+username+';PWD='+ password) as conn:
    ...
    while True:
        ...
        for row in rows:
            ...
            importData.append({"idFilm": row[0], "primaryTitle": row[1], "startYear": row[2]})

        create_nodes(graph.auto(), importData, labels={"Film"})
        exportedCount += len(rows)
        print(f"{exportedCount}/{totalCount} title records exported")
    ...
    while True:
        ...
        for row in rows:
            importData.append({"idArtist": row[0], "primaryName": row[1], "birthYear": row[2]})

        create_nodes(graph.auto(), importData, labels={"Artist"})
        exportedCount += len(rows)
        print(f"{exportedCount}/{totalCount} artists exported")
    ...
    while True:
        ...
        for cat in importData:
            ...
            create_relationships(graph.auto(), importData[cat], relation_type, start_node_key=("Artist", "idArtist"), end_node_key=("Film", "idFilm"))
        exportedCount += len(rows)
        print(f"{exportedCount}/{totalCount} relationships exported")
